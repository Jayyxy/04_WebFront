...
// 변수에 대입한 값 변경하기
number1 = 300 ; // number1에 300 재대입 (기존 10값에 덮어쓰기)
number2 = 400 ; // number2에 400 재대입

// 변경된 두 변수의 값 출력 
// 1) 문자열 + 문자열/숫자/변수 == 이어쓰끼
console.log("number1의 값 : " + number1); // "number1 의 값 : 3"
console.log("100" + 100); // "100100"

// 2) 괄호 내부에 , 를 작성해 각각의 값을 독릭적으로 
// 출력하기
// 형변환 없이 출력되어 문자열 따로, 뭐시기
// 따로 독립되어 출력됨 
...
// 블록 레벨 scope / 함수레벨 scope

// 블록 레벨 scope (let, const)
// let 과 const 로 선언된 변수는
// 변수가 선언된 블록 ( 중괄호 {} ) 내부에서만 유효하다는 것을 의미
...
console.log(foo1) // 전역변수이기 때문에 ( 블록과 관계없음 )
console.log(foo2) // 전역변수이기 때문에 ( 블록과 관계없음 )
// console.log(num1) // 에러 
// console.log(num2) // 에러
...
// var 는 일반 블록레벨만 무시할 뿐
// 함수레벨 블록은 무시하지 못함.
// 즉, 함수 블록 아넹서 선언된 var 변수는
// 함수 안에서만 사용 가능하다 
...
// 에러 안나고 undefined가 찍힘.
// -> 호이스팅 되어 var a; 선언부는 최상단으로 올라감.
// -> 선언은 되어있으나 a에 값이 대입되지 않았기 때문에
// undefined 를 반환하는 것임. 
...
// 2. let / const의 호이스팅
// : let/const 로 선언된 변수는 선언만 해당
// 스코프의 최상단으로 호이스팅됨 .
// var와 달리, let/const로 선언된 변수는 초기화가 이루어지기
// 전에는 사용될 수 없음.
...
// -> x 의 선언이 호이스팅 됨. 그러나 TDZ에 놓여 초기화 전까지 접근 불가 
...
console.log("numbers 배열 값 중 0번째 :", number[0],
  typeof number[0]);

console.log("numbers 배열 값 중 0번째 :", number[1],
  typeof number[1]);

console.log("numbers 배열 값 중 0번째 :", number[2],
  typeof number[2]);

console.log("numbers 배열 값 중 0번째 :", number[3],
  typeof number[3]);
